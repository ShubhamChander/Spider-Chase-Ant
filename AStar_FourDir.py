...
#This heuristic will choose path that has the smallest distance; either
# smallest x difference or y difference
class AStar_FourDir():
	BFS=BFSLogic()
	...
	def calcHValue(spiderLocCurr, antLoc):
		...
		differenceX = abs(x1-x2)
		differenceY = abs(y1-y2)
	...
	def applyFourDir(self,spiderLoc,antLoc):
		...
		while True:
			...
			for i in range(len(possMoves)):
				succ = possMoves[i]
				FourDist_H = AStar_FourDir.calcHValue(succ,antLoc)
				#print("The depth used is ", depth)
				FValue = AStar_FourDir.calcFValue(depth,FourDist_H)
